...
public class ZhongzHouseController extends BaseController
{
	...
	@PostMapping("/list")
	@ResponseBody
	public Result<ZhongzHouse> list(ZhongzHouseParam zhongzHouseParam)
	{
	 ...
	 * 查询某用户有效租期内房屋列表（需要登录）
	 */
	@ApiOperation(value="查询某用户有效租期内房屋列表（需要登录）")
	@PostMapping("/listUserHouses")
	@ResponseBody
	public Result<ZhongzHouse> listUserHouses(ZhongzUserOrderParam zhongzUserOrderParam)
	{
		return zhongzHouseService.selectZhongzUserHouseList(zhongzUserOrderParam);
	}
